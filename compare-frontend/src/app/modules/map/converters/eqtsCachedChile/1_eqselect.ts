...
@Injectable()
export class CachedEqSelectionChile implements Converter {
    ...
    constructor(
        ...
        this.riesgosState$.pipe(
            filter(state => !!state),
            map(state => {
                const leftScenarioData = state!.scenarioData.ChileCached!.left!;
    ...
    applies(scenario: ScenarioName, step: string): boolean {
        return scenario === "ChileCached" && step === "selectEqChile";
    }
    ...
    makeLayers(state: RiesgosScenarioState, data: RiesgosProductResolved[], partition: PartitionName): Observable<LayerComposite[]> {
        ...
        const stepState = state.steps.find(s => s.step.id === "selectEqChile")?.state.type;


        if (stepState === StepStateTypes.available) {
            const availableEqs = state.products.find(p => p.id === "userChoiceChile");
            ...
            if (availableEqs) {
                ...
                layers.push({
                    ...
                    onClick(location: number[], features: Feature[]) {
                        ...
                        _store.dispatch(stepConfig({ partition: state.partition, scenario: state.scenario, stepId: "selectEqChile", values: { userChoiceChile: feature } }));
                    },
                    onHover() {},
                    opacity: 1.0,
                    visible: true,
        ...
        if (stepState === StepStateTypes.completed) {
            const selectedEq = data.find(d => d.id === "selectedEqChile");
            const _trslt = this.translate;
