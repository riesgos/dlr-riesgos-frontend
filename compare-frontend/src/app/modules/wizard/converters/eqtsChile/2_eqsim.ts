...
export class WizardEqSimulationChile implements Converter {
    ...
    getInfo(state: RiesgosScenarioState, data: RiesgosProductResolved[]): WizardComposite {
        ...
        const vsgrid = state.products.find(p => p.id === "vsgridChile")!;
        const gmpe = state.products.find(p => p.id === "gmpeChile")!;
