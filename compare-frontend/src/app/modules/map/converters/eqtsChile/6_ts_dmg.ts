...
export class TSDmgChile implements Converter {

    applies(scenario: ScenarioName, step: string): boolean {
        return scenario === "ChileShort" &&  step === "TsDamageChile";
    }

    makeLayers(state: RiesgosScenarioState, data: RiesgosProductResolved[]): Observable<LayerComposite[]> {
        const wmsProduct = data.find(p => p.id === "tsDamageWmsChile");
        if (!wmsProduct) return of([]);
        const summaryProduct = data.find(d => d.id === "tsDamageSummaryChile");
        if (!summaryProduct) return of([]);
        
        const schema = data.find(d => d.id === "schemaTsChile")?.value || "Medina_2019";

        const wms = new URL(wmsProduct.value);

        const dmgLayer: LayerComposite = {
            id: "TsDamage-WMS-damage",
            stepId: "TsDamageChile",
            layer: new TileLayer({
                source: new TileWMS({
