...
} from 'src/app/components/config_wizard/wizardable_products';
...
import {
    ...
    toDecimalPlaces,
    yellowRedRange,
} from 'src/app/helpers/colorhelpers';
import {
...
export class EtypeChile implements WizardableProductAugmenter {
    ...
    makeProductWizardable(
        product: RiesgosProductResolved
    ): StringSelectUserConfigurableProduct[] {
        return [
            {
                ...product,
                    ...
                    vectorLayerAttributes: {
                        ...
                        detailPopupHtml: (properties) => {
                            ...
                            const selectedProperties = {
                                ...
                                Id: regexTransform(
                                    properties['origin.publicID']
                                ),
                            };
                            if (properties['origin.time.value']) {
                                const date = new Date(
                                    Date.parse(properties['origin.time.value'])
                                );
