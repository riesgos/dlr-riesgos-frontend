...
export class LayersComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    if (this.data.oneLayerOnly) {
      const nrControlables = this.data.layerControlables.length;
      const nrLayersVisible = this.data.layerControlables.filter(lc => lc.visible).length;
      if (nrLayersVisible > 1 || nrControlables <= 0) {
        const firstCompositeId = nrControlables > 0 ? this.data.layerControlables[0].id : undefined;
