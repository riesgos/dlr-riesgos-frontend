...
import {
    ...
    WizardableProductAugmenter,
    WizardableStepAugmenter,
} from 'src/app/services/augmenter/augmenter.service';
import { DataService } from 'src/app/services/data/data.service';
import { getProduct } from '../../riesgos.selectors';
import {
    RiesgosProduct,
    RiesgosProductResolved,
    RiesgosStep,
} from '../../riesgos.state';
...
export class InitialExposureChile implements MappableProductAugmenter {
    private metadata$ = this.store.select(getProduct('exposureMetaChile')).pipe(
        shareReplay(),
        switchMap((p) => {
            if (p) {
        ...
        return [
            {
                ...
                toUkisLayers: (
                    ownValue: any,
                    mapSvc: MapOlService,
                    layerSvc: LayersService,
                    httpClient: HttpClient,
                    store: Store,
                    layerMarshaller: LayerMarshaller
                ) => {
                    ...
                    return combineLatest([layers$, metadata$]).pipe(
                        map(([layers, metaData]) => {
                            ...
                            exposureLayer.dynamicDescription = globalSummary(
                                product.value[0]
                            );

                            exposureLayer.legendImg = dynamicLegend(
                                product.value[0]
                            );
    ...
    makeStepWizardable(step: RiesgosStep): WizardableStep {
        return {
            ...
            wizardProperties: {
                ...
                dataSources: [
                    {
                        label: 'Gomez-Zapata et al., 2021',
                        href: 'https://dataservices.gfz-potsdam.de/panmetaworks/showshort.php?id=8c3ba5ec-26b3-11ec-9603-497c92695674',
                    },
